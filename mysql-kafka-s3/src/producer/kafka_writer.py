...
def produce_data_to_kafka_topics():
    try:
        ...
        # dim_employee record to be sent on kafka
        ...
        dim_employee=dim_employee.filter(F.col("salesRepEmployeeId")>last_sent_emp_id)
        logging.info(f"Number of employee record will be sent to kafka: [{dim_employee.count()}]")
        dim_employee.show()

        #dim_location to be sent on kafka
        ...
        # dim_product to be sent on kafka
        ...
        dim_product=df_product.select(
                    F.col(df_product.productCode),
                    F.col(df_product.productName),
                    F.col(df_product.productScale),
                    F.col(df_product.productVendor),
                    F.col(df_product.productDescription),
                    F.col(df_product.quantityInStock),
                    F.col(df_product.buyPrice),
                    F.col(df_product.MSRP),
                    F.col(df_product.productCreatedTimestamp)
                )        

        logging.info(f"Total number of total product: {dim_product.count()}")
        dim_product = dim_product.filter(F.col('productCreatedTimestamp')>latest_sent_created_product)
        logging.info(f"Number of product record to be sent on kafka: [{dim_product.count()}]")
        dim_product.show()

        # dim_date to be sent on kafka
        ...
        #dim_date=dim_date.select(dim_date.columns).withColumn('dateId',F.col(dim_date.date))
        logging.info(f"Number of record we have after removing duplicates from dimension date table: [{dim_date.count()}]")

         # preparing for the fact table 
        ...
        query = f"""
                        ...
                        ord.{table.orders.orderDate} as dateId,
                        ord.{table.orders.status} as Status,
                        ordDetails.{table.orderDetails.quantityOrdered}  as quantity,
                        ordDetails.{table.orderDetails.priceEach } as unitPrice
        ...
        # dim_customer to be sent on kafka
        ...
        dim_customer=df_cust.select(
            F.col(df_cust.customerNumber).alias("customerId"),
            F.col(df_cust.customerName),
            F.col(df_cust.contactFirstName),
            F.col(df_cust.contactLastName),
            F.col(df_cust.phone),
            F.col(df_cust.creditLimit),
            F.col(df_cust.addressLine1),
            F.col(df_cust.addressLine2)
        )        
        logging.info(f"Total customer in database [{dim_customer.count()}]")
        dim_customer = dim_customer.filter(F.col("customerId")>last_sent_customer_id)
        print(f"Number of  customerrecord to be sent on kafka: [{dim_customer.count()}]")
        dim_customer.show()
        ...
        last_customerNumber = dim_customer.agg({"customerId":"max"}).take(1)[0][0]
